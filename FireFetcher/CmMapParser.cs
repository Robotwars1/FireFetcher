...
namespace FireFetcher
{
    internal class CmMapParser
    {
        private readonly Dictionary<string, string> MapDictionary = new()
        {
            ...
            { "Several Chambers", "several chambers" }
        };

        public string ParseMap(object MapValue)
        {
            return MapDictionary.ElementAt(MapDictionary.Values.ToList().IndexOf(MapValue.ToString())).Key;
        }
    }
}
